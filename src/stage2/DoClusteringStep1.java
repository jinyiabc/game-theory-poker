...
public class DoClusteringStep1 {
		...
		while (numBoardCards <= 4) {
			...
			for(int i = 0; i < (Card.NUM_CARDS-1); i++) {
				...
				for(int j = i+1; j < Card.NUM_CARDS; j++) {
					...
					String inPath = inputDir + Integer.valueOf(i).toString() + "_" + Integer.valueOf(j).toString();
	...
	private static short[] getTopScoreGroups(ArrayList<ToupleFloatInt> recs, int numBoardCards) {
