...
public class SimpleBot implements Player {
    ...
    */
   public synchronized Action preFlopAction() {
      ...
      if (c1.getRank() == c2.getRank()) {
         if (c1.getRank() >= Card.TEN || c1.getRank() == Card.TWO) {
            return Action.raiseAction(toCall, gi.getBetSize());
         }
      ...
      if (gi.getAmountToCall(ourSeat) <= gi.getSmallBet()) {
         if (Math.random() < 0.05) {
            return Action.callAction(toCall);
         }
      }
   ...
   private Action postFlopAction() {
      ...
      for (int i=0; i<gi.getNumSeats(); i++) {
         if (gi.inGame(i)) { // if a player is in seat i
            if (i!= ourSeat && gi.getPlayerInfo(i).isCommitted()) {
               numCommitted++;
            }
         }
      }
      
      debug(" | HRn = " + Math.round(HRN*10) + 
            " PPot = "+ Math.round(PPOT*10) + 
            " PotOdds = " + Math.round(PO*10) +
      ...
      if (toCall == 0) { 
         if (Math.random() < HRN*HRN) {
            return Action.betAction(gi.getBetSize()); // bet a hand in proportion to it's strength
         }
         if (Math.random() < PPOT) {
            return Action.betAction(gi.getBetSize()); // semi-bluff
         }
         // just check
